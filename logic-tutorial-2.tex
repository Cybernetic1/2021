...
\cc{The central tenet of my theory is that the state $\vect{x}$ of the AI system is consisted of \emp{logic propositions} and that $\vect{F}$ plays the role of the \emp{logic consequence} operator $\vdash$:
}{
...
\cc{So our goal now is to elucidate the structure of $\vdash$.  Currently the most elegant formulation is given by \emp{categorical logic} or \emp{topos theory}.
}{
...
In the following pages, I will outline a complete logical theory for AGI, and the central idea of this theory is Curry-Howard isomorphism....
}
...
Curry-Howard isomorphism is a very profound thought. If you are not careful, you may even think that it is equivalent to not being said.
}
...
Simply put: When we think logically, there is a syntax form on the surface, namely $A \Rightarrow B$:
}
\begin{equation}
...
And at the bottom of this grammar, there is an \emp{operation}, which can be regarded as the work of executing \emp{proof} (proof), which maps the proof of $A$ to the proof of $B$ (for To avoid symbolic cumbersomeness, I will record these ``proof witnesses'' as $\witness$, but each of them is different).
}

\cc{一个传统的数学函数，例如 $f(x) = x + 2$ 用我们惯常的符号表示为：
}{
A traditional mathematical function, such as $f(x) = x + 2$ is represented by our usual notation:
}
\begin{equation}
\begin{aligned}
...
It's not new either. But if the \textbf{proposition} of "$x$ is even" is regarded as a \textbf{type} or \textbf{set} with a \textbf{proof} (witness), this view is new :
}
\begin{equation}
\begin{aligned}
...
This is called the \emp{Brouwer-Heyting-Kolmogorov (BHK) interpretation}. Because this idea is relatively subtle, it has been repeatedly discovered many times. The name can include: Brouwer-Heyting-Kolmogorov-Sch\"{o}nfinkel -Curry-Meredith-Kleene-Feys-G\"{o}del-L\"{a}uchli-Kreisel-Tait-Lawvere-Howard-\mbox{de Bruijn}-Scott-Martin-L\"{o} f-Girard-Reynolds-Stenlund-Constable-Coquand-Huet-Lambek...
}

\cc{根据 HoTT (homotopy type theory)，一个命题 可以\textbf{有}或\textbf{没有}证明； 如果有，则它的证明都是一样的，所以 经典逻辑命题 只可以取值 \textbf{真}或\textbf{假}。 我的理论推断： 模糊逻辑的取值 $\in [ 0,1 ]$ 是因为 fuzzy 命题的「证明」可以有很多个，而 fuzzy 命题的 \textbf{真值}是由 支持／反对 的证明的\textbf{比例}决定的（见 \S\ref{sec:fuzzy-logic}）。 
}{
According to HoTT (homotopy type theory), a proposition can be proved \textbf{yes} or \textbf{not}; if it has, its proofs are the same, so classical logic propositions can only take the value \textbf{true} or \textbf{false}. My theoretical inference: The value of fuzzy logic $\in [0,1]$ is because there can be many "proofs" of fuzzy propositions, and the \textbf{true value} of fuzzy propositions are proved by support/against Determined by \textbf{ratio} (see \S\ref{sec:fuzzy-logic}).
}
...
\end{equation}
\cc{从另一角度看，Curry-Howard isomorphism 可以看成是 某些 \emp{状态}（states，例如 $A$）和状态之间的 \emp{转换} （transitions，例如 $\stackrel{f}{\mapsto}$）之间的 \textbf{对偶}。 而这种 对偶 不断在 截然不同的范畴里出现：
}{
From another perspective, Curry-Howard isomorphism can be regarded as some \emp{states} (states, such as $A$) and \emp{transitions} (transitions, such as $\stackrel{f}{ \textbf{dual} between \mapsto}$). And this duality continues to appear in completely different categories:
}
\begin{equation}
...
The first two are Curry-Howard, the third is added by Lambek, and the rest are from John Baez \& M. Stay's paper: \textit{Physics, Topology, Logic and Computation: a Rosetta stone} [2010]. For example, in physics, it is the duality of Hilbert space and operators; in topology, the famous example of cobordism is this ``pair of pants'':
}
\begin{equation}
...
In string theory, it represents the "time course" of the strings above becoming the strings below.
}
...
The language to describe \emp{program} or \emp{computation} is called type theory. For example, there can be a sentence like this in a general programming language:
}
\begin{equation}
\begin{small}
...
It means that length() is a function, input String, output Integer.
}

\cc{在数学里 我们描述 \emp{函数} 时会用：
}{
In mathematics, we use when describing \emp{function}:
}
\begin{equation}
f: A \rightarrow B
\end{equation}
...
This expression is actually the general form of type theory:
}
\begin{equation}
...
And this notation $t:T$ can actually be written as $t \in T$ (but not orthodox only).
}

\cc{换句话说，types 就是 \textbf{集合}，terms 是集合中的 \textbf{元素}。 
}{
In other words, type is \textbf{collection}, and terms are \textbf{elements} in the collection.
}

\cc{更一般地，一个 type theory 的句子 可以包含 type \emp{context}：
}{
More generally, a type theory sentence can contain type \emp{context}:
}
\begin{equation}
...
It means like ``declare'' the type of some variables at the beginning of program, and then program can be assigned the type after \emp{}.
}

\cc{这个 $\vdash$ 的过程 称为 \emp{type assignment}，而这就是 type theory 做的全部工作。 
}{
This $\vdash$ process is called \emp{type assignment}, and this is all the work done by type theory.
}
...
In a program, besides defining the type, you also need to define \emp{function}. This work is done by $\lambda$-calculus.
}

\cc{$\lambda$-calculus 可以定义函数 而不需要提及它的「名字」。 例如，用数学式表达：
}{
$\lambda$-calculus can define functions without mentioning its "name". For example, using mathematical expressions:
}
\begin{equation}
f(x) \triangleq x^2
\end{equation}
...
$\lambda$-calculus was invented by Alonso Church to study the properties of \emp{substitution} in mathematics. Substitute is something every middle school student knows to do, but it is surprisingly troublesome to express it in mathematics.
}
...
At the same time, Church found that $\lambda$-calculus is a "universal" calculation form, which is equivalent to \emp{Turing machines}. "Father of AI" John McCarthy used $\lambda$-calculus to develop the \emp{Lisp} language, which is the originator of all functional programming languages.
}
...
Under Curry-Howard, type $A$ is the logical proposition $A$, and the element in type $A$ or set $A$ is its \textbf{proof} (proof, or proof witness).
}
...
And, $A \Rightarrow B$ is also a logical proposition, which corresponds to the function type $A \rightarrow B$, which can also be written as $B^A$, and the elements in this type or set are some functions $f: A \ rightarrow B$. If such a function exists, then type $A \rightarrow B$ has "inhabited", in other words $A \Rightarrow B$ has \textbf{proof}.
}
...
Curry-Howard isomorphism reveals the relationship between type theory and \emp{intuitionistic logic} (intuitionistic logic). The characteristic of this logic is that there is no \emp{law of excluded middle} (law of excluded middle, LEM), or equivalently, \emp{double negation}, that is, $\neg \neg p \Rightarrow p$.
}

\cc{排中律 是说： $p \vee \neg p$ 是 恒真命题。 但在 直觉主义 逻辑中，$p \vee \neg p$ 表示 $p$ 的证明 \textbf{或} $\neg p$ 的证明，但有时候这两者都不知道（例如 现时仍未找到证明，或者不可能找到证明）。 
}{
The law of excluded middle says: $p \vee \neg p$ is the proposition of Hengzhen. But in intuitionistic logic, $p \vee \neg p$ means the proof of $p$ \textbf{or} the proof of $\neg p$, but sometimes neither of them is known (for example, no proof has been found yet , Or impossible to find proof).
}

\cc{附带一提： 人们惊讶地发现，在直觉主义逻辑下，axiom of choice $\Rightarrow$ law of excluded middle.  换句话说，axiom of choice 和 直觉主义 也有内在的矛盾。 
}{
Incidentally: People are surprised to find that under the logic of intuitionism, axiom of choice $\Rightarrow$ law of excluded middle. In other words, axiom of choice and intuitionism also have inherent contradictions.
}
...
In topology, \emp{open sets} is generally used to represent subsets in space (this habit originated from the Hausdorff period). But the \emp{complement} $\overline{p}$ of $p$ is not open, so you should define $\neg p$ as the \emp{interior} of the complement of $p$, that is, $\neg p \triangleq \overline{p}^{\, \circ}$. So $p \cup \neg p \neq \mbox{Universe}$: \footnote{diagram from the book: \textit{Classical and Non-classical Logics - an introduction to the mathematics of propositions} [Eric Schechter 2005], p.126.}
}
\begin{equation}
...
Propositional logic means: only propositions, but ignore any \textbf{internal proposition} structure.
}

\cc{假设 $p, q$ 是命题，命题逻辑的基本运算 就是 $p \wedge q, p \vee q, p \Rightarrow q, \neg p$.
}{
Assuming that $p, q$ are propositions, the basic operations of propositional logic are $p \wedge q, p \vee q, p \Rightarrow q, \neg p$.
}

\cc{First-order logic 的意思是： 容许 这样的方法 构成 命题：
}{
First-order logic means: allow such methods to constitute the proposition:
}
\begin{equation}
...
Predicate means \emp{predicate}; predicate is some "propositions with holes", they become complete propositions after being filled in objects. Similarly, there can be predicates of \emp{multiple}, for example:
}
\begin{equation}
\mbox{Loves} (\mbox{John}, \mbox{Mary}).
\end{equation}
\cc{First-order 指的是： $\forall, \exists$ 这些 \emp{量词} 可以 \textbf{作用} 在 objects 的类别上，例如（Mary \textit{人见人爱}）：
}{
First-order refers to: $\forall, \exists$ These \emp{quantifiers} can be \textbf{act} in the category of objects, for example (Mary \textit{人见人爱}):
}
\begin{equation}
...
But first-order logic does not allow quantifiers to act on the category of predicates, unless second-order logic is used.
}
...
Note that $p$ is quantified above the category of predicates.
}
...
The history of Type theory can be traced back even earlier. It originated from Russell in order to solve \emp{logical paradox}, for example: "\textit{A barber who only cuts hair for a person who doesn’t cut his hair can help himself with a haircut?}" The root of these logical paradoxes lies in: the same definition When writing something, \textbf{refers to} the thing itself halfway through. This bad definition is called \emp{impredicative}. In order to avoid bad definitions, everything must be "declared" before its type appears. This is the original purpose of type theory.
}
...
Before Curry-Howard isomorphism was not taken seriously, there was a simpler way to define logic with type theory. In this method, the logical propositions $p, q, p \wedge q$ etc. \textbf{ are defined directly by} terms, instead of the logical proposition = types, proof = terms like Curry-Howard.
}

\cc{在这情况下 type theory 处理的是 (first- or higher-order) predicate logic 的方面。 这是说，例如：
}{
In this case, type theory deals with (first- or higher-order) predicate logic. This means, for example:
}
\begin{equation}
...
Inside IsHuman is a function term, it inputs an object, and outputs whether it is the truth value of "person" $\in \Omega = \{ \top, \bot \}$. Therefore IsHuman is a type of $ \mbox{Obj} \rightarrow \Omega$ term.
}

\cc{
\cc{这种做法没有容纳 Curry-Howard isomorphism 的馀地。 如果要做到后者，需要的是 Martin-L\"{o}f type theory....}{
}{
This approach has no room for Curry-Howard isomorphism. If you want to do the latter, you need Martin-L\"{o}f type theory....}{
}
\cc{This approach leaves no room to accommodate Curry-Howard isomorphism.  To do the latter, we would need Martin-L\"{o}f type theory....
}{
...
According to Curry-Howard, the following $A {\color{blue}\Rightarrow} B$ is a logical proposition and therefore a type:
}
\begin{equation}
...
But on the other hand, Human() and Mortal() these two predicates also need to use type theory to form propositions. They are also types. ${\color{red}\mbox{red} \rightarrow}$ and ${\color{ The two levels of blue}\mbox{蓝} \Rightarrow}$ are completely different, but they are forced together because of Curry-Howard. This makes type theory seem like "one mind can't do two things."
}

\cc{在 ``simple'' type theory 里面可以 构造：
}{
It can be constructed in ``simple'' type theory:
}
\begin{itemize}
\cc{	\item sum type $A + B$
}{
\item sum type $A + B$
}
\cc{	\item product type $A \times B$
}{
\item product type $A \times B$
}
\cc{	\item function type $A \rightarrow B$
}{
...
They correspond to the intuitionistic logic of $\vee, \wedge, {\color{blue}\Rightarrow}$. These are at the level of \textbf{propositional logic} and have "exhausted" the magic weapon of type theory.
}

\cc{但 Human(Socrates) 也是由 Human() 和 Socrates 构成的命题，这构成的方法是用一个 arrow ${\color{red}\rightarrow}$，但已经没有 arrow 可用。 
}{
But Human(Socrates) is also a proposition composed of Human() and Socrates. The method of this formation is to use an arrow ${\color{red}\rightarrow}$, but there is no arrow available.
}

\cc{Martin-L\"{o}f 提出的解决方案是 引入新的 \emp{type constructors}：
}{
The solution proposed by Martin-L\"{o}f is to introduce a new \emp{type constructors}:
}
\begin{itemize}
\cc{	\item \textbf{dependent} sum type $\Sigma$
}{
\item \textbf{dependent} sum type $\Sigma$
}
\cc{	\item \textbf{dependent} product type $\Pi$
}{
...
The type of $B$ in Dependent sum $\displaystyle \sum_A B$ depends on $A$. The result of + for the entire family of A becomes similar to product $A \times B$.
}

\cc{Dependent product $\displaystyle \prod_A B$ 里面 $B$ 的类型 depends on $A$. 整个 family of A 的 $\times$ 的结果变成类似 exponentiation $B^A$.
}{
The type of $B$ in Dependent product $\displaystyle \prod_A B$ depends on $A$. The result of $\times$ for the entire family of A becomes similar to exponentiation $B^A$.
}
...
A very beautiful result is: If you use $\sum_A B$ and $\prod_A B$ to define logical propositions, these types, if inhabited, correspond to $\exists A. B(A)$ and $\forall A, respectively . B(A)$. This is because: If $A \times B$ is inhabited, it means that \textbf{exists} at least one $B(A)$; and if $B^A$ is inhabited, there is a function that will \textbf{arbitrary} $A$ send to $B$.
}

\cc{Per Martin-L\"{o}f (1942-) was the first logician to see the full importance of the connection between intuitionistic logic and type theory.
}{
...
Many people know that in classical logic $\wedge, \vee$ corresponds to \textbf{arithmetic operation} $\times, +$ (it can also be regarded as fuzzy logic $\min, \max$.) In fact, this is George Boole tried to turn \emp{logic} into some kind of \emp{algebra} reason.
}

\cc{较少人知道的是 $A \Rightarrow B$ 也对应於 $B^A$： \footnote{其中 $0^0$ 是「不确定式」，但按照 组合学 惯例可以定义为 1.}
}{
What less people know is that $A \Rightarrow B$ also corresponds to $B^A$: \footnote{where $0^0$ is "indeterminate", but according to the combinatorics convention, it can be defined as 1.}
}
\begin{equation}
...
This surprising "coincidence" seems to prove once again that Curry-Howard correspondence is correct; in particular, it means that $\Rightarrow$ should be regarded as \textbf(function), the so-called ``functional interpretation of logical deduction.''
}
...
To observe in more detail, the truth values of $A$ and $B$ in the table (\ref{truth-table:material-implication}) can be regarded as whether their types are \textbf{inhabitants}. Type $A$’s inhabitant It is its proof $\witness$, and no proof is $\emptyset$. Or it can be generalized to: the truth value of the proposition $A$ = the type of $A$ as the set \textbf{cardinality}; $|A|$. In this way, the true value of $A \Rightarrow B$ is $|B^A|$, that is, from $\{\witness\}$ or $\emptyset$ to $\{\witness\ }$ or $\emptyset$ map \textbf{number}, and this map is only empty set when $\emptyset \mapsto \{\witness\}$ (impossible).
}

\cc{这个观察 可以推广到 fuzzy logic (\S\ref{sec:fuzzy-implication}) 和 strict implication $A \strictif B$ (\S\ref{sec:strict-implication}).
}{
This observation can be extended to fuzzy logic (\S\ref(sec:fuzzy-implication)) and strict implication $A \strictif B$ (\S\ref(sec:strict-implication)).
}

\cc{但 material implication 导致某种悖论：
}{
But material implication leads to a certain paradox:
}
\begin{equation}
...
That is to say, any two occurrences of \textbf{simultaneously} will lead to a conclusion similar to \textbf{causality}, but this causal relationship may not be established. The reason for this fallacy seems to be the confusion of \emp{cases} at different times. In other words: We should verify all the cases of table (\ref{truth-table:material-implication}) before drawing conclusions Say $A \Rightarrow B$; However, according to the material implication of classical logic, only one case is needed to conclude that $A \Rightarrow B$.
}

\cc{那么，这个谬误 为什么没有在 经典逻辑 AI 系统中被发现？
}{
So, why is this fallacy not found in the classic logic AI system?
}
...
The idea of this kind of generators is the same as generators of ideals, groups, function fields, etc. in mathematics.
}

\cc{Machine learning 的目的，是求得一组这样的 generators，而它生成的\textbf{机制}，即是逻辑推导 $\vdash$.
}{
The purpose of machine learning is to obtain a set of such generators, and the \textbf{mechanism} it generates is a logical derivation $\vdash$.
}

\cc{但由於在 learning 的过程中，需要验证 \textbf{不同时间}的 cases，因此避免了 太轻易接受 $A \Rightarrow B$ 的问题。 
}{
However, since the cases of \textbf{different time} need to be verified during the learning process, the problem of accepting $A \Rightarrow B$ too easily is avoided.
}
...
Corresponding type theory to category theory, this work was done by Lambek, thus completing Curry-Howard-Lambek's "Trinity":
}
...
The important significance of Topos is that it is a category that can be used for \textbf{for logical operations}. The key is that it can express the concept of \textbf{sub-objects}, or more generally called \emp{sub-objects}.
}
...
A topos $\mathcal(C)$ has sub-object classifer $\Omega$ in it, so that $X \rightarrow \Omega \cong \mbox{sub-objects of} X$. In other words, a subset of $X$ can be used $X \rightarrow \Omega$ This mapping comes from \emp{represent}.
}

\cc{在 $\mathbf{Set}$ 这个 topos 里面，$\Omega$ 是一个有\textbf{两个}元素的集合，可以记作 $\{ \top, \bot \}$.  那么 $X \rightarrow \Omega$ 就是一些 \emp{命题}，例如 $X$ 是人的集合，则 $\begin{tikzcd}[column sep = 12ex]
}{
In the topos of $\mathbf{Set}$, $\Omega$ is a set of \textbf{two} elements, which can be written as $\{ \top, \bot \}$. Then $X \rightarrow \ Omega$ are some \emp{propositions}, for example, $X$ is a collection of people, then $\begin{tikzcd}[column sep = 12ex]
}
\cc{X \arrow[r, "\mathrm{mathematician}"] & \Omega \end{tikzcd}$ 定义哪些人是数学家。 
}{
X \arrow[r, "\mathrm{mathematician}"] & \Omega \end{tikzcd}$ Define who is a mathematician.
}
...
\item $X \stackrel{!}{\longrightarrow} 1$ is a unique arrow, which maps the collection $X$ \textbf{entirely} to 1. And 1 is a terminal object, which is defined as leading to it There can only be one arrow.
}

\cc{	\item $\begin{tikzcd}1 \arrow[r, "\mathrm{true}"] & \Omega \end{tikzcd}$ 在 $\top$ 和 $\bot$ 之间选择 $\top$, 因此叫 ``true'' arrow.
}{
\item $\begin{tikzcd}1 \arrow[r, "\mathrm{true}"] & \Omega \end{tikzcd}$ choose between $\top$ and $\bot$ $\top$, so Call it ``true'' arrow.
}

\cc{	\item $\begin{tikzcd}X \arrow[r, tail, "m"] & Y \end{tikzcd}$ 是 \emp{monic} arrow，特别地，在 $\mathbf{Set}$ 里面它就是 \emp{inclusion} map，即 $\begin{tikzcd}X \arrow[r, hook, "m"] & Y \end{tikzcd}$.  它表示 $X$ 是 $Y$ 的\textbf{子集}，$X \subseteq Y$.
}{
\item $\begin{tikzcd}X \arrow[r, tail, "m"] & Y \end{tikzcd}$ is \emp{monic} arrow, especially, in $\mathbf{Set}$ it is \emp{inclusion} map, ie $\begin{tikzcd}X \arrow[r, hook, "m"] & Y \end{tikzcd}$. It means that $X$ is a \textbf{ subset of $Y$ }, $X \subseteq Y$.
}

\cc{	\item $\begin{tikzcd}Y \arrow[r, "\Chi_m"] & \Omega \end{tikzcd}$ 是集合论中熟悉的 \emp{characteristic function}, 当元素 $e \in X \subseteq Y$ 时，$\Chi(e)$ 取值 1，否则为 0.  正是 $\Chi_m$ 的存在令这幅图 commute.  $\Chi_m$ 也记作 $\ulcorner m \urcorner$.
}{
\item $\begin{tikzcd}Y \arrow[r, "\Chi_m"] & \Omega \end{tikzcd}$ is the familiar \emp{characteristic function} in set theory, when the element $e \in X \subseteq For Y$, $\Chi(e)$ takes the value 1, otherwise it is 0. It is the existence of $\Chi_m$ that makes this picture committee. $\Chi_m$ is also recorded as $\ulcorner m \urcorner$.
}
\end{itemize}
\cc{不熟悉基本范畴论的读者，我非常推荐看一看《Conceptual Mathematics》这本书，写得连中学生也可以看懂，而作者之一的 Lawvere 正是 topos 理论的创始人。 
}{
For readers who are not familiar with basic category theory, I highly recommend reading the book "Conceptual Mathematics", which is written so that even middle school students can understand it. One of the authors, Lawvere, is the founder of topos theory.
}
...
The importance of the Topos theory is that it uses the language of category \textbf{restatement} the entire basis of set theory. In particular, the symbols in logic, such as $P(x), \forall x, \exists x,$ seem to be unable to be expressed by category theory on the surface. This is Lawvere's amazing achievement.
}
\begin{equation}
\nonumber
...
Look again Figure (\ref(eqn:subobject-classifier)):
}
\begin{equation}
\begin{tikzcd}[column sep = normal]
...
The ${\color{red}\mathrel{\substack{1\\\downarrow\\\Omega}}}$ on the right is called \emp{generic subobject}. Its \emp{pull back} square uses ${\ The color{red}\lrcorner}$ sign indicates. And the $\mathrel{\substack{X\\\downarrow\\Y}}$ on the left is a general sub-object. We say that the \textbf{property} of being a sub-object is \emp{stable under pullbacks}.
}

\subsection{The idea of classifying spaces}

\cc{Topos 中 $Y \rightarrow \Omega$ 这个箭头，是 \emp{classifying space} 或 \emp{moduli space} 的一个特例。  这方面我不太熟悉。
}{
The arrow $Y \rightarrow \Omega$ in Topos is a special case of \emp{classifying space} or \emp{moduli space}. I am not very familiar with this aspect.
}

\cc{A moduli space is a space whose points can be put in a 1-to-1 correspondence with the objects we are interested in.
}{
A moduli space is a space whose points can be put in a 1-to-1 correspondence with the objects we are interested in.
}

\cc{A moduli space is a manifold, or variety, which parametrises some class of
}{
A moduli space is a manifold, or variety, which parametrises some class of
}
\cc{geometric objects.  For example:
}{
...
\cc{Every family over $B$ is the pullback of $\mathcal{C}$ via a unique map from $B$ to $\mathcal{M}$:
}{
...
\end{tikzcd}
\end{equation}
\cc{Classifying space for principal $G$-bundles:
}{
...
\cc{Let $\mbox{Forms}(\bar{X})$ denote the set of formulas with only the variables $\bar{X}$ free. ($\bar{X}$ may contain multiple variables.)
}{
Let $\mbox{Forms}(\bar{X})$ denote the set of formulas with only the variables $\bar{X}$ free. ($\bar{X}$ may contain multiple variables.)
}

\cc{Then one can always trivially add an additional \textbf{dummy} variable $Y$:
}{
...
\end{equation}
\cc{taking each formula $\Phi(\bar{X})$ to itself.
}{
taking each formula $\Phi(\bar{X})$ to itself.
}

\cc{It turns out that $\exists$ and $\forall$ are \textbf{adjoints} to the map $\delta$:
}{
...
\end{tikzcd}
\end{equation}
\cc{or simply denoted as $\exists \dashv \delta \dashv \forall$.  \cc{而这是十分 makes sense 的，因为 一个 $\Phi(\bar{X},Y)$ 的式子，经过 $\forall Y. \; \Phi(\bar{X},Y)$ 之后，就变成一个和 $Y$ \textbf{无关}的式子。}
}{
or simply denoted as $\exists \dashv \delta \dashv \forall$. \cc{ and this makes sense, because a $\Phi(\bar{X},Y)$ formula passes through $\ forall Y. \; After \Phi(\bar{X},Y)$, it becomes a formula that has nothing to do with $Y$ \textbf{. }
}
\cc{{This makes a lot of sense, because a formula $\Phi(\bar{X},Y)$, after being quantified as $\forall Y. \; \Phi(\bar{X},Y)$, turns into a formula that is \textbf{independent} of $Y$.}
}{
...
\cc{另一个方法是 借助 $T$ 定义}
\cc{{Another way is to define with the help of a variable set $T$}
}{
...
\cc{{(Note that $\bar{X}$ and $Y$ are show as 2D domains below) :}
}{
...
\cc{{Note that in (\ref{fig:Lawvere-cylindrification-exists}), $\delta$ is a projection from $\bar{X} \times Y \rightarrow X$, but the $\delta$ in (\ref{eqn:Abramsky-1}) can be \textbf{any} map $Y \rightarrow X$, and this seems to be the most general definition of $\forall$ and $\exists$.  The advantage of using categorical definitions is that they can be easily transferred to other categories, such as Hilbert space.}
}{
...
\cc{类似地有 $\forall$ 的定义：}
\cc{{Similarly we have the definition of $\forall$:}
}{
...
This is logically accustomed and nothing unusual. It infers that $A \Rightarrow B$ can replace $A \vdash B$. Since $\vdash$ is \textbf{neural network} in our AI system, this It means that the "black box" knowledge in the neural network can be "externalized" (externalize) into \textbf{logical propositions}, which is of key importance in intelligent systems, because it means that knowledge can be obtained through language learning ( Although this is not necessarily a category theory to be able to see it.)
}
...
\cc{How sheaves gives rise to representables.
}{
...
A logical proposition $\phi$ can be regarded as a function of a certain domain $A \stackrel{\phi}{\rightarrow} \Omega$, where $\Omega = \{ \top, \bot \}$.
}
...
For example, the condition $c$ can "force" the set $G$ to have at least 100 prime numbers, or at least 10,000 prime numbers. Denoted as $c \Vdash P$, $P$ is a logical proposition.
}
...
\begin{center}
\cc{	\rule{0.8\textwidth}{1pt} \\
}{
\rule{0.8\textwidth}{1pt} \\
}
\cc{	以下部分 包含很多未落实的想法，包括我自己的 ``thinking aloud'' 
}{
...
Regardless of intensional or extensional, this interpretation method is in harmony with HoTT theory (\S\ref{sec:HoTT}): the \textbf{proof} of the concept "John" is $J_1 \wedge J_2 ... \wedge J_n \ Rightarrow$ John, where $J_i$ is John’s \textbf{attribute}. Similarly, the proof of "handsome" is $H_1 \wedge H_2 ... \wedge H_n \Rightarrow$ handsome. Therefore, the fuzzy truth value of ``John is handsome'' is $\{ J_i \}$ and $\{ H_i A measure of the commonality (such as intersection or union) of \}$.
}
...
Here $A$ and $B$ are \textbf{logical propositions}, such as "I am a student"; $f$ maps the witness $\witness$ in the $A$ proposition to the $B$ proposition. Note: The information on the \textbf{syntax} of "I am a student" is represented by the \emp{domain} and \emp{co-domain} of $f$.
}
