...
\cc{（我觉得这个可能是 Yoneda lemma 的一个应用，但根据 MathOverflow 上一些专家的看法，只是表面上有点相似。另一个（有证明的）例子是 \textbf{continuation-passing style} 是 类型 $a$ 和 $(a \rightarrow r) \rightarrow r$ 之间的等价关係，但我仍未看懂 ....）
	...
	(I think this may be an application of Yoneda lemma, but according to some experts on MathOverflow, the resemblance is only superficial.  Another (proven) example is \textbf{continuation-passing style} as an equivalence between the types $a$ and $(a \rightarrow r) \rightarrow r$;  But I still don't fully understand it...)
}
...
\cc{也可以说：命题 $\phi(x)$ 是真的，其中 $x$ 是 $A$ 的\emp{元素}。
	...
	It can also be said that the proposition $\phi(x)$ is true, where $x$ is an \emp{element} of $A$.
}
