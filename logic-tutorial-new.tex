...
\section*{Summary}
\begin{itemize}
\cc{\item 用比较简单的方法讲解 范畴论下逻辑的结构，这亦叫 categorical logic.  这是目前数学上描述逻辑结构的最满意的形式。 了解逻辑的抽象结构之后，可以将它应用到 AGI 或神经网络的设计上。 
}{
\item Outlines the mathematical structure of logic as formulated using category theory, the latest and most satisfactory approach so far.  This mathematical knowledge may help the AGI designer if they want to endow neural networks with logic structure, if this is deemed desirable.
}
\end{itemize}
...
The Curry-Howard isomorphism expresses a \textbf{connection} between logic and functions, specifically functions that map proofs to more proofs, thus implementing inference as a \textbf{process}.  The correspondence \uline{cannot be proven mathematically};  Its status in mathematics is akin to the postulates of \textbf{quantum mechanics}:  they relate certain mathematical structures, such as Hilbert space vectors and Hermitian operators, to \textbf{physical reality}.  Without these postulates, the Schr\"{o}dinger equation is just a differential equation.  Analogously, the Curry-Howard isomorphism connects mathematical structures to \textbf{logic}:
\begin{equation}
\begin{aligned}
\boxed{\mbox{mathematics}} & \ \stackrel{\mbox{\scriptsize quantum theory}}{\longrightarrow} && \boxed{\mbox{physical reality}} \\
\boxed{\mbox{mathematics}} & \ \stackrel{\mbox{\scriptsize Curry-Howard theory}}{\longrightarrow} && \boxed{\mbox{logic}}
\end{aligned}
\end{equation}
Here logic is viewed as some kind of \textit{empirical} phenomenon, whose rules are not known to us \textit{a priori}, but are distilled from our experience.

\cc{简单来说： 当我们做 逻辑思考时，表面上有一种语法上 (syntax) 的形式，即 $A \Rightarrow B$：
}{
	Simply put: logic is a \textbf{syntax} on the surface, for example the formula $A \Rightarrow B$:
}
\begin{equation}
\label{eqn:Curry-Howard}
\begin{aligned}
\boxed{\mbox{logic}} \qquad \underdash{$A \Longrightarrow B$} & \\
\boxed{\mbox{program}} \qquad \witness \; \stackrel{f}{\longmapsto} \; \witness \hspace*{10pt} &
\end{aligned}
\end{equation}
\cc{而在这 语法「底下」，还有一个 \emp{运算}，它可以看成是执行 \emp{证明} (proof) 的工作，它将 $A$ 的证明 map 到 $B$ 的证明。 \footnote{为了避免符号累赘，我将这些 ``proof witness'' 都记作 $\witness$，但它们每个是不同的。}
}{
	And underneath this syntax, there is an \emp{operation}, which can be regarded as executing a \emp{proof}, which maps the proof of $A$ to the proof of $B$. \footnote{To avoid clutter, I will denote all ``proof witnesses'' as $\witness$, even though each of them is different.}
}
...
\cc{也不是新的。 但如果将「$x$是偶数」这个\textbf{命题}，看成是一个\textbf{类型}或\textbf{集合}，里面有个\textbf{证明} (witness)，这个看法是新的： 
	...
	is also not new.  But to regard the \textbf{proposition} ``$x$ is even'' as a \textbf{type} or \textbf{set} with a \textbf{proof witness}, this perspective has new substance:
}
\begin{equation}
\begin{aligned}
...
Once we associate a mathematical function to the logic symbol $\Rightarrow$, such a correspondence induces a lot of other correspondences:  propositions are abstract spaces, and as spaces they may possess topological structure, ... and so on.  Thus the Curry-Howard isomorphism induces an entire enterprise of applying mathematics to the study of logic.  
...
\cc{根据 HoTT (homotopy type theory)，一个命题 可以\textbf{有}或\textbf{没有}证明； 如果有，则它的证明都是一样的，所以 经典逻辑命题 只可以取值 \textbf{真}或\textbf{假}。 我的理论推断： 模糊逻辑的取值 $\in [ 0,1 ]$ 是因为 fuzzy 命题 可以有「部分」的证明（见 \S\ref{sec:fuzzy-logic}）。 
	...
	According to HoTT (homotopy type theory), a proposition can be proven \textbf{true} or \textbf{false};  if it is true, its proofs are all the same, so classical logic propositions can only take the value \textbf{true} or \textbf{false}.  My theory posits that the truth value of fuzzy logic $\in [0,1]$ because there can be ``partial proofs'' of fuzzy propositions (see \S\ref{sec:fuzzy-logic}).
}
...
\subsubsection{\texorpdfstring{$\lambda$}{lambda}-calculus}
...
Y \arrow[r, swap, "\chi_m"] & {\color{red}\Omega}
\end{tikzcd}
\end{equation}
...
According to topos theory, (\ref{eqn:proposition-in-topos}) is also a \textbf{subobject} of $\Gamma$.  For example, the predicate ``Male'' is a subobject of the domain of ``Humans''.

\subsubsection{Interpreting \texorpdfstring{$\Rightarrow$}{Implication Arrow}}
...
\subsection{\texorpdfstring{$\forall$}{For-all} and \texorpdfstring{$\exists$}{Exist} as adjunctions}
...
\subsection{\texorpdfstring{$\wedge$}{∧} and \texorpdfstring{$\Rightarrow$}{⇒} as product-hom adjunction}
...
C & \qquad & C \nonumber
\end{eqnarray}
\cc{但如果左边加多一个式子 $A \wedge B \wedge C \Rightarrow D$，则可以推出新的结论 $D$：
}{
...
B & \quad \vdash \quad & B \\
C & \qquad & C \nonumber \\
& \hspace*{-8.6em} {\color{red}A \wedge B \wedge C \Rightarrow D} \qquad & D \nonumber
\end{eqnarray}
...
\cc{$\vdash$ 的功能和 $\Rightarrow$ 类似，但 $\vdash$ 是 \textbf{元逻辑} (meta-logic) 的符号，而 $\Rightarrow$ 是逻辑\textbf{之内}的算符。
	...
	The role of $\vdash$ is similar to $\Rightarrow$, but $\vdash$ is a \textbf{meta-logic} symbol, whereas $\Rightarrow$ is an operator within logic.
}

\cc{在 (\ref{eqn:deduction}) 中，${\color{red} A \wedge B \wedge C \Rightarrow D}$ 导致了 $A \wedge B \wedge C \vdash D$ 的出现。
}{
	In (\ref{eqn:deduction}), ${\color{red} A \wedge B \wedge C \Rightarrow D}$ led to the appearance of $A \wedge B \wedge C \vdash D$.
}
...
\subsection{Classifying topos \texorpdfstring{$\leftrightharpoons$}{⇄} internal language}
...
Unlike set theory, topos theory allows more \textbf{models}, which is the reason behind Cohen's proof's success.
...
As a special case, the sheaf over the singleton $\{*\}$ is the $\mathbf{Set}$ topos:
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.7]{Set-topos-as-sheaf-over-singleton.png}}}
\end{equation}

In general, a sheaf is built upon a \emp{site} which is a topological space with underlying set $X$ and its set of opens $\mathcal{O}(X)$. The subobject classifier $\Omega$ depends on an open set $U$ chosen from $\mathcal{O}(X)$.  In general, $\Omega$ is the ``opens of $U$''.  For example, in the special case of the $\mathbf{Set}$ topos above, $\Omega$ is the opens of $\{*\}$ which is either $\emptyset$ or $\{*\}$.  Therefore the $\Omega$ in $\mathbf{Set}$ is $\{0, 1\}$.

The sheaves defined in this way are automatically toposes and they are known as \emp{Grothendieck toposes}.
...
The important thing to remember is:  in lattice theory, the order $a \le b$ usually corresponds to $a \Rightarrow b$ in logic.  Notice the arrow direction is reversed \footnote{I don't know the precise reason for this, perhaps this is just a matter of convention.}.  In the lattice, we can form new elements such as $a \wedge b$ and $a \vee b$, these of course correspond to compound logic propositions formed via logic connectives.  The lattice shows us the \textbf{implication order} ($\Rightarrow$) among such propositions.  For example, the following are Boolean lattices of 2 and 3 elements \footnote{I have not carefully examined why some elements are included or not.}:
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.7]{Boolean-lattice.png}}}
\qquad
\vcenter{\hbox{\includegraphics[scale=0.7]{Boolean-lattice-eg1.png}}}
\end{equation}
The above are examples of \emp{Hasse diagrams}, with edges showing the $\le$ order.  Notice that not every $\le$ relation is shown;   the Hasse diagram only shows those edges where $a \le b$ ``immediately'', ie. the ``cover'' relation:
\begin{equation}
a \mbox{ covers } b \quad \mbox{means} \quad a \le b \mbox{ ``immediately''}
\end{equation}

\cc{以下的 \emp{拓樸}模型 和 \emp{格} (lattice) 模型，都是 Heyting algebra 的模型： 
}{
	The following \emp{topological} model and \emp{lattice} model are both models of Heyting algebra:
}
\cc{
	\begin{equation}
	\boxed{
	...
	\boxed{
		\begin{aligned}
		\mbox{rain} & \\
		& \boxed{\mbox{grass wet}} \\
		& \boxed{\mbox{no tennis}} \; 
		\end{aligned}
	} \quad \quad
	\vcenter{\hbox{
			\begin{tikzpicture}[scale=0.8]
			\node[label={[xshift=0em, yshift=-0.1em]\mbox{rain}}] (a) at (1,0) {\textbullet};
			\node[label={[xshift=-1em, yshift=-2.22em]\mbox{grass wet}}, swap] (b) at (0,-2) {\textbullet};
...
\cc{两者之间是等价的，起源於 \emp{Stone duality} (every Boolean algebra is isomorphic to a topology of open sets)，其后再被推广到 \emp{Priestley} 拓扑对偶 等，都是大同小异的。
	...
	The two are equivalent from \emp{Stone duality} (every Boolean algebra is isomorphic to a topology of open sets), and then generalized to \emp{Priestley} topological duality, etc., all of which are similar .
}

A lattice can be equivalently turned into an algebra via:
\begin{equation}
a \le b \quad \Leftrightarrow \quad  a * b = a.
\end{equation}
...
\cc{注意： $\mbox{下雨} \Heytingarrow \mbox{草地湿}$ 是 Heyting implication，这个 $\Heytingarrow$ 的存在 并不是因为「下雨」的\textbf{真值} 比「草地湿」的\textbf{真值 小}。 
	...
	Note: $\mbox{rainy} \Heytingarrow \mbox{grass wet}$ is a Heyting implication.  This $\Heytingarrow$ arrow does not mean the \textbf{true value} of ``rainy'' is larger than that of ``grass wet''.
}
...
\cc{可以用以下例子说明 Heyting implication $\Heytingarrow$ 的定义：
	...
	The following example may illustrate the definition of Heyting implication $\Heytingarrow$:
}
\cc{
	\begin{equation}
	...
	\begin{equation}
	\forall X. \quad X \subseteq (\highlight{\mbox{rain} \Heytingarrow \mbox{grass wet}}) \quad \mbox{iff} \quad (X \wedge \mbox{rain}) \subseteq \mbox{grass wet}
	\end{equation}
}
\cc{
	\begin{equation}
	...
	\begin{equation}
	\vcenter{\hbox{\includegraphics[scale=0.8]{../2020/topology-Heyting-rain-wet(En).png}}}
	\end{equation}
}

\cc{经典逻辑的 $\Rightarrow$ 和 直觉主义逻辑 $\Heytingarrow$ 只有 在\textbf{边界}上的 微小差异：
}{
	The classical $\Rightarrow$ and the intuitive $\Heytingarrow$ have only minor differences on the \textbf{boundary}:
}
\begin{align}
...
\cc{根据 Stone duality, 既然可以用 Boolean 或 Heyting algebra 做逻辑推导，因此 也可以用 toplogy of (open) sets 做逻辑推导。 其根据就是 以下的等价：
}{
...
\begin{equation}
A \vdash B \quad \Leftrightarrow \quad A \subseteq B
\end{equation}

\cc{这种 topology 可以视为 \textbf{truth table} 的一种形式，如下图所示，每个\textbf{区域}代 表 真值表的\textbf{一行}：
}{
	This kind of topology can be regarded as a form of \textbf{truth table}, as shown in the following figure, where each \textbf{area} represents a \textbf{row} of the truth table:
}
\begin{equation}
\begin{tabular}{|c|c|c|c|}
\hline 
...
\cc{另一点发现是，在 \S\ref{sec:material-implication} 所说的 material implication 问题，在 intuitionistic logic 里仍然存在。
	...
	Another insight is that the material implication problem in \S\ref{sec:material-implication} still exists in intuitionistic logic.
}
...
\section{Logic's connections with algebras}

[ These sections will be expanded later... ]

\subsection{Gr\"{o}bner bases}

In the book \textit{``Term Rewriting and All That''} [1998, Ch. 8], it is mentioned that the Knuth-Bendix \textbf{completion procedure} [1970, first correctness proof by Huet in 1981] for rewriting logic is roughly the same as \textbf{Buchberger's algorithm} [1965] for finding Gr\"{o}bner bases for polynomial rings.  The completion procedure takes as input a set of rewriting rules and tries to find an equivalent rewriting system that is \textbf{convergent} (ie. guaranteed to terminate), or return failure.  In computational algebra, Gr\"{o}bner bases [first discovered by Hironaka \cc{（弘中平佑）}{} in 1964] are used to decide the \textbf{ideal congruence} and \textbf{ideal membership} problems in polynomial rings.  The ideal congruence problem can be seen as a \textbf{word problem}, and Gr\"{o}bner bases define convergent reduction relations on polynomials.

\subsection{BCI / BCK algebras}

There is another, perhaps related, and perhaps even more important connection:  If we apply the Curry-Howard correspondence to ``Hilbert-style'' proofs, we find that proof elements behave as \textbf{combinators}, as in combinatory logic.  This gives rise to \textbf{BCI algebra}, a kind of non-associative algebra capturing the syntax of $\rightarrow$.  The name BCI comes from the combinators B, C, I, K, etc.  These algebras are non-associative but they also have close connections with associative algebras....

Reference: \parencite{Sorensen2006}

\subsection{Representation theory}

Using techniques from representation theory, one starts with a \textbf{quiver} $Q$, which is a kind of graph, and gets a \textbf{path algebra} $k Q$ and its vector-space \textbf{representation}.  The path algebra is a vector space with all possible paths (of lengths $\ge 0$) forming its basis, and with multiplication given by path concatenation.  For its representation, each vertex is seen as a vector space and each path is a linear transformation between the spaces.  In other words, a representation of a quiver $Q$ is an association of an $R$-module to each vertex of $Q$, and a morphism between each module for each arrow.

But this technique only works for \textbf{associative} algebras.
...
\begin{equation}
\cc{\mbox{瑞士在非洲} \Rightarrow \mbox{猪会飞}}
   {\mbox{Switzerland is in Africa} \Rightarrow \mbox{pigs can fly}}
\end{equation}
...
\begin{equation}
\begin{tikzcd}[column sep = normal]
X \arrow[r, "!"] \arrow[d, tail, swap, "m"] & \mathbb{1} \arrow[d, "\zeta"] \\
Y \arrow[r, swap, "\chi_m"] & \Omega
\end{tikzcd}
\end{equation}
where the subobject classifier $\Omega$ is now a \textbf{lattice}.  $\zeta$ is a fuzzy ``distribution''.  This commutative diagram is similar to the one for \emp{moduli space}, where every family over a base scheme $B$ arises from the \textbf{pullback} of an identity map $\mathrel{\substack{\mathcal{C} \\ \ \downarrow \mathbb{1} \\ \mathcal{M}}}$ where $\mathcal{M}$ is the moduli space:
\begin{equation}
\begin{tikzcd}[column sep = normal]
\mathcal{D} \arrow[r] \arrow[d, swap, "\phi"] & \mathcal{C} \arrow[d, "\mathbb{1}"] \\
...
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/fuzzy-interval-as-lattice.png}}}
\end{equation}
This is a category with $\subseteq$ as the $\le$ morphism.

Even more figuratively, we can actually think of the human head with a topology of open sets:
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/fuzzy-baldness.png}}}
\end{equation}
This gives a very intuitive interpretation of the fuzzy degree: as the \textbf{ratio} of the has-hair area to the full area.
...
Consider this (rather simplistic) \textbf{binary} logic formula for recognizing a ``dog'':
\begin{equation}
\mbox{dog-head} \wedge \mbox{dog-body} \wedge \mbox{dog-tail} \Rightarrow \mbox{dog}
\end{equation}
From the Curry-Howard view, each atomic proposition corresponds to an abstract space representing its type:
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/dog-head.png}}}
\; \wedge \;
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/dog-body.png}}}
\; \wedge \;
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/dog-tail.png}}}
\; \Rightarrow \;
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/dog.png}}}
\end{equation}
In binary logic, each type-space is either inhabited or empty:
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/dog-head-2.png}}}
\; \wedge \;
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/dog-body-2.png}}}
\; \wedge \;
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/dog-tail-2.png}}}
\; \Rightarrow \;
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/dog-2.png}}}
\end{equation}
We want to extend this to the fuzzy case.  Each proof will have a \textbf{fuzzy degree} which is an element from the $[0,1]$ lattice, which we represent by sub-spaces of the type-spaces:
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/dog-head-3.png}}}
\; \wedge \;
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/dog-body-3.png}}}
\; \wedge \;
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/dog-tail-3.png}}}
\; \Rightarrow \;
\vcenter{\hbox{\includegraphics[scale=0.7]{../2022/dog-3.png}}}
\end{equation}
where we interpret $\wedge$ as $\min()$, so the fuzzy truth value of the conclusion is the minimum among the antecedents.  The above picture seems compatible with Voevodsky's HoTT, where fuzzy sets are at level 0, and the identity of elements are expressed by propositions (level -1).  Indeed, the fuzzy type-space may have a metric structure allowing to \textbf{measure} areas.
...
On the proposition level, each type (space) either has a proof or is empty.  Any two proofs on this level are considered \textbf{identical}.  Thus, any 2 elements in a type on this level is connected by a homotopy (contractable space).  

On the set level, the identity between 2 elements is a proposition $a \stackrel{?}{=} b$;  it can be true or false.  Thus a set can have multiple distinct elements.
