...
\cc{（我觉得这个可能是 Yoneda lemma 的一个应用，但根据 MathOverflow 上一些专家的看法，只是表面上有点相似。
	...
	(I think this may be an application of Yoneda lemma, but according to some experts on MathOverflow, the resemblance is only superficial.)
}
...
\cc{当 $\mathcal{C}^{\mathrm{op}}$ = topology of open sets 时，我觉得下图特别容易理解：
	...
	For $\mathcal{C}^{\mathrm{op}}$ = the topology of open sets, I think the following figure is helpful for understanding:
}
\begin{equation}
