...
\cc{在 $\mathbf{Set}$ 这个 topos 里面，$\Omega$ 是一个有\textbf{两个}元素的集合，可以记作 $\{ \top, \bot \}$.  那么 $X \rightarrow \Omega$ 就是一些 \emp{命题}，例如 $X$ 是人的集合，则 $\begin{tikzcd}[column sep = 12ex] X \arrow[r, "\mathrm{mathematician}"] & \Omega \end{tikzcd}$ 定义哪些人是数学家。 
	...
	In the topos of $\mathbf{Set}$, $\Omega$ is a set of \textbf{two} elements, which can be written as $\{ \top, \bot \}$. Then $X \rightarrow \Omega$ are some \emp{propositions}, for example, $X$ is a collection of people, then $\begin{tikzcd}[ampersand replacement=\&, column sep = 12ex]
...
\begin{itemize}
	...
	\cc{	\item $\begin{tikzcd}[ampersand replacement=\&] 1 \arrow[r, "\mathrm{true}"] \& \Omega \end{tikzcd}$ 在 $\top$ 和 $\bot$ 之间选择 $\top$, 因此叫 ``true'' arrow.
		...
		\item $\begin{tikzcd}[ampersand replacement=\&] 1 \arrow[r, "\mathrm{true}"] \& \Omega \end{tikzcd}$ choose between $\top$ and $\bot$ $\top$, so Call it ``true'' arrow.
	}
	...
	\cc{	\item $\begin{tikzcd}[ampersand replacement=\&] X \arrow[r, tail, "m"] \& Y \end{tikzcd}$ 是 \emp{monic} arrow，特别地，在 $\mathbf{Set}$ 里面它就是 \emp{inclusion} map，即 $\begin{tikzcd}[ampersand replacement=\&] X \arrow[r, hook, "m"] \& Y \end{tikzcd}$.  它表示 $X$ 是 $Y$ 的\textbf{子集}，$X \subseteq Y$.
		...
		\item $\begin{tikzcd}[ampersand replacement=\&] X \arrow[r, tail, "m"] \& Y \end{tikzcd}$ is \emp{monic} arrow, especially, in $\mathbf{Set}$ it is \emp{inclusion} map, ie $\begin{tikzcd}[ampersand replacement=\&] X \arrow[r, hook, "m"] \& Y \end{tikzcd}$. It means that $X$ is a \textbf{ subset of $Y$ }, $X \subseteq Y$.
	}
	...
	\cc{	\item $\begin{tikzcd}[ampersand replacement=\&] Y \arrow[r, "\Chi_m"] \& \Omega \end{tikzcd}$ 是集合论中熟悉的 \emp{characteristic function}, 当元素 $e \in X \subseteq Y$ 时，$\Chi(e)$ 取值 1，否则为 0.  正是 $\Chi_m$ 的存在令这幅图 commute.  $\Chi_m$ 也记作 $\ulcorner m \urcorner$.
		...
		\item $\begin{tikzcd}[ampersand replacement=\&] Y \arrow[r, "\Chi_m"] \& \Omega \end{tikzcd}$ is the familiar \emp{characteristic function} in set theory, when the element $e \in X \subseteq For Y$, $\Chi(e)$ takes the value 1, otherwise it is 0. It is the existence of $\Chi_m$ that makes this picture committee. $\Chi_m$ is also recorded as $\ulcorner m \urcorner$.
	}
\end{itemize}
