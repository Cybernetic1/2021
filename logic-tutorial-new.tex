...
\cc{例如，以下的 \emp{拓樸}模型 和 \emp{格} (lattice) 模型，都是 Heyting algebra 的模型： 
}{
	For example, the following \emp{topological} model and \emp{lattice} model are both models of Heyting algebra:
}
\cc{
\begin{equation}
\boxed{
...
The relation $\le$ defines the ``cover'' relation:
\begin{equation}
a \mbox{ covers } b \quad \mbox{means} \quad a \le b \mbox{ ``immediately''}
\end{equation}
and this defines also the \emp{Hasse diagram}.
...
\cc{注意： $\mbox{下雨} \Heytingarrow \mbox{草地湿}$ 是 Heyting implication，这个 $\Heytingarrow$ 的存在 并不是因为「下雨」的\textbf{真值} 比「草地湿」的\textbf{真值 小}。 
	...
	Note: $\mbox{rainy} \Heytingarrow \mbox{grass wet}$ is Heyting implication. The existence of this $\Heytingarrow$ is not because the \textbf{true value} of "rainy" is better than "grass wet" \textbf{true value is small}.
}
