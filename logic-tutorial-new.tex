...
\end{tikzcd}
\end{equation}
where the subobject classifier $\Omega$ should be replaced by a \textbf{lattice}.  $\zeta$ is a fuzzy distribution.  This is analogous to the idea of \emp{moduli space}, where every family over a base scheme $B$ comes from the \textbf{pullback} of an identity map $\mathrel{\substack{\mathcal{C} \\ \ \downarrow \mathbb{1} \\ \mathcal{M}}}$ where $\mathcal{M}$ is called the moduli space:
\begin{equation}
\begin{tikzcd}[column sep = normal]
...
\end{equation}
where we interpret $\wedge$ as $\min$ to shade the areas.  The above picture seems compatible with Voevodsky's HoTT.
